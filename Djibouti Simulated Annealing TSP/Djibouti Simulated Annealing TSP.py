# -*- coding: utf-8 -*-
"""
Created on Sat Jun 13 14:28:17 2020
...
# Upload Djibouti Travelling Sales Man Problem city coordinates data
djibouti_data = pd.read_table('Djibouti_TSP.txt', header = None, sep = '\s+')
djibouti_data.columns = ['city', 'x', 'y']
...
for i in djibouti_data["city"]:
    for j in djibouti_data["city"]:
        row = djibouti_data[djibouti_data["city"] == j][["x", "y"]]
        x_coord = row["x"].tolist()[0]
        y_coord = row["y"].tolist()[0]
        djibouti_data.loc[djibouti_data['city'] == i, j] = ((djibouti_data["x"] - x_coord)**2 + (djibouti_data["y"] - y_coord)**2)**0.5

df = djibouti_data.drop(["x", "y"], axis=1)
...
X0 = (np.array(djibouti_data["city"]))
rd.shuffle(X0)
